...
The plaintext token never traverses SSM. The helper selects only the live
Kubernetes context whose Zed API pod advertises the expected public registry
URL, then validates a newly minted token against that exact pod before
returning RSA-OAEP ciphertext to the runner.
"""
...
def validate_inputs(token_name: str, expected_url: str) -> None:
    if not TOKEN_NAME_RE.fullmatch(token_name):
        raise RuntimeError("invalid bounded token name")
    if not PUBLIC_URL_RE.fullmatch(expected_url):
        raise RuntimeError("invalid expected public registry URL")


def selector_shell(expected_url: str) -> str:
    expected = shlex.quote(expected_url.rstrip("/"))
    return f'''EXPECTED_PUBLIC_BASE_URL={expected}
  ...
  try_kubeconfig() {{
    ...
    while IFS= read -r context; do
      ...
      pod=$(kubectl --kubeconfig "$config" --context "$context" -n zed \\
        get pods -l app=dd-zed-api-server -o jsonpath='{{.items[0].metadata.name}}' 2>/dev/null || true)
      [ -n "$pod" ] || continue
...
def send_ssm(instance_id: str, region: str, command: str) -> dict:
    with tempfile.TemporaryDirectory(prefix="den2050-ssm-") as tmp:
        ...
        for _ in range(120):
            ...
            status = probe.stdout.strip() if probe.returncode == 0 else "Pending"
...
def revoke(instance_id: str, region: str, token_name: str, expected_url: str) -> None:
    validate_inputs(token_name, expected_url)
    q_name = shlex.quote(token_name)
    remote = f'''set -euo pipefail
{selector_shell(expected_url)}
work=$(mktemp -d /tmp/den2050-zed-revoke.XXXXXX)
trap 'rm -rf "$work"' EXIT
printf 'stage=context_selection\\n'
select_zed_context
printf 'stage=revoke_token\\n'
if ! zkubectl -n zed exec "$ZED_API_POD" -c zed-api-server -- \\
    /usr/local/bin/zed-api-server revoke-token --name {q_name} >"$work/out" 2>"$work/err"; then
...
def mint(instance_id: str, region: str, token_name: str, token_file: Path, expected_url: str) -> None:
    validate_inputs(token_name, expected_url)
    with tempfile.TemporaryDirectory(prefix="den2050-zed-token-") as tmp:
        ...
        q_name = shlex.quote(token_name)
        q_key = shlex.quote(base64.b64encode(public_key.read_bytes()).decode("ascii"))
        remote = f'''set -euo pipefail
{selector_shell(expected_url)}
umask 077
...
created=$(zkubectl -n zed exec "$ZED_API_POD" -c zed-api-server -- \\
  /usr/local/bin/zed-api-server create-token --name {q_name} --expires-in-days 1)
token=$(printf '%s\\n' "$created" | tail -n 1)
...
[ "$status" = 404 ] || {{ echo "local minted-token probe returned HTTP $status" >&2; exit 1; }}
...
'''
        try:
            ...
            try:
                revoke(instance_id, region, token_name, expected_url)
        ...
        matches = [
            line.split("=", 1)[1]
            for line in (invocation.get("Stdout") or "").splitlines()
...
def main() -> int:
    ...
    for target in (mint_parser, revoke_parser):
        ...
        target.add_argument("--expected-public-base-url", default=DEFAULT_REGISTRY)
    mint_parser.add_argument("--token-file", type=Path, required=True)
    args = parser.parse_args()
    if args.command == "mint":
        mint(args.instance_id, args.region, args.token_name, args.token_file, args.expected_public_base_url)
    else:

...
usage() {
  cat <<'USAGE'
Usage: preflight_gha_clone_webhook.sh [--namespace NAME] [--expect-active] [--probe-live]
...
configured but inert. --expect-active instead requires the reviewed budget
pilot state: one replica for each service and all three execution flags true.
Both modes verify immutable images, split Secret authority, exact repository
and workflow rules, AWS-only router placement, Services, and NetworkPolicies.
Secret values are never decoded or printed.

--probe-live requires one available clone-server replica and one available
router replica, opens local port-forwards, and verifies both /healthz and
/readyz against the selected inert or active mode. It sends no webhook and
performs no Kubernetes write.
USAGE
}
...
[[ "$(env_literal "$clone_container" GHA_CLONE_EXECUTION_ENABLED)" == "$expected_execution" ]] || fail "clone API execution does not match $mode_name mode"
[[ "$(env_literal "$clone_container" GHA_CLONE_WEBHOOK_EXECUTION_ENABLED)" == "$expected_execution" ]] || fail "clone webhook execution does not match $mode_name mode"
...
[[ "$(env_literal "$router_container" GHA_EXECUTOR_ROUTER_EXECUTION_ENABLED)" == "$expected_execution" ]] || fail "router execution does not match $mode_name mode"
...
require_secret_binding "$clone_container" GHA_CLONE_GITHUB_TOKEN "$clone_secret" github_token
...
pass "clone and router Deployments are digest-pinned, tokenless, non-root, read-only, and $mode_name"
...
[[ "$clone_replicas" == "$expected_replicas" && "$router_replicas" == "$expected_replicas" ]] ||
  fail "$mode_name preflight requires clone/router replicas=$expected_replicas, found $clone_replicas/$router_replicas"
if [[ "$probe_live" != true ]]; then
  pass "static read-only preflight completed in $mode_name mode at replicas=$expected_replicas"
  exit 0
fi

[[ "$clone_replicas" == 1 && "$router_replicas" == 1 ]] ||
  fail "--probe-live requires clone/router replicas=1; select --expect-active for the deployed pilot"
[[ "$(jq -r '.status.availableReplicas // 0' <<<"$clone_deployment")" -ge 1 ]] || fail 'clone server has no available replica'
[[ "$(jq -r '.status.availableReplicas // 0' <<<"$router_deployment")" -ge 1 ]] || fail 'executor router has no available replica'
...
jq -e --argjson expected "$expected_execution" '
  .ok == true and .executionEnabled == $expected and .webhookExecutionEnabled == $expected and
...
' "$work_dir/clone-health.json" >/dev/null || fail "clone health response is not fully configured for $mode_name mode"
jq -e '.ok == true and .executionReady == true' "$work_dir/clone-ready.json" >/dev/null || fail "clone ready response rejected $mode_name mode"
jq -e --argjson expected "$expected_execution" '
  .ok == true and .executionEnabled == $expected and .configuredExecutors == 2 and
...
pass "live clone and router health/readiness passed in $mode_name mode"

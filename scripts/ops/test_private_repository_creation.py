...
class PrivateRepositoryCreationTests(unittest.TestCase):
    ...
    def test_existing_public_repository_is_rejected_without_post(self) -> None:
        api = ScriptedApi(
            [
                (
    ...
    def test_existing_repository_identity_mismatch_is_rejected_without_post(self) -> None:
        api = ScriptedApi(
            [
                (
                    ("GET", self.repository_path, None),
    ...
    def test_raised_409_create_race_reconciles_exact_private_repository(self) -> None:
        reconciled = private_repository()
        api = ScriptedApi(
            [
                (("GET", self.repository_path, None), (404, None)),
                (
                    ("POST", self.organization_path, create_payload()),
                    RuntimeError(
                        "GitHub API 409 for POST /orgs/example/repos: conflict"
                    ),
                ),
                (("GET", self.repository_path, None), (200, reconciled)),
            ]
    ...
    def test_similar_runtime_error_for_different_path_is_not_swallowed(self) -> None:
        api = ScriptedApi(
            [
                (("GET", self.repository_path, None), (404, None)),
                (
                    ("POST", self.organization_path, create_payload()),
                    RuntimeError(
                        "GitHub API 422 for POST /orgs/other/repos: already exists"
                    ),
                ),
            ]
        )

        with self.assertRaisesRegex(RuntimeError, "/orgs/other/repos"):
            self.ensure(api, [])
        api.assert_exhausted()

    def test_non_runtime_api_exception_is_not_swallowed(self) -> None:
        api = ScriptedApi(
            [
                (("GET", self.repository_path, None), (404, None)),
                (
                    ("POST", self.organization_path, create_payload()),
                    ValueError("transport decoding failed"),
                ),
            ]
        )

        with self.assertRaisesRegex(ValueError, "transport decoding failed"):
            self.ensure(api, [])
        api.assert_exhausted()

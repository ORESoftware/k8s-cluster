...
NEW = '''def ensure_tag(api: GitHub, spec: RepositorySpec, main_sha: str) -> str:
    ...
    if observed is None:
        ...
        status, _ = api.patch(
            f"/repos/{spec.full_name}/git/refs/tags/{encoded_tag}",
            {"sha": main_sha, "force": True},
        )
        if status != 200:
            raise RuntimeError(
                f"reconcile bootstrap {tag} failed for {spec.full_name}: HTTP {status}"
            )
        for attempt in range(12):
            observed = tag_ref(api, spec, tag)
            if observed == main_sha:
    ...
    if observed != main_sha:
        raise RuntimeError(
            f"bootstrap initial tag mismatch for {spec.full_name}: {observed} != {main_sha}"
        )
    return observed
'''
...
def patch(path: Path) -> str:
    ...
    if old_count == 0 and new_count == 1:
        ...
        raise RuntimeError(
            "refusing unexpected Elenkos bootstrap tag contract: "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Reconcile v0.1.0 only inside the exact bootstrap-managed Elenkos fleet. "
            "The publisher verifies the allowlisted repository and bootstrap marker "
    ...
    print(
        "ELENKOS_BOOTSTRAP_TAG_RECONCILE_PATCHED "
        f"path={args.publisher} result={result} tag=v0.1.0 bounded=true"
    )
    return 0

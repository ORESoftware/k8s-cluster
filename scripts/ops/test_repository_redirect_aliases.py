...
def exact_repository(full_name: str) -> dict[str, object]:
    return {
        "full_name": full_name,
        "private": True,
        "visibility": "private",
...
class RepositoryRedirectAliasTests(unittest.TestCase):
    ...
    def test_create_only_helper_preserves_redirect_then_creates_exact_canonical_name(self) -> None:
        ...
        created = exact_repository("StreemPilot/streempilot-flutter-app")
        api = ScriptedApi(
            [
                (("GET", repository_path, None), (200, redirect_repository(self.actual))),
                (
    ...
    def test_incomplete_same_owner_mismatch_is_not_treated_as_redirect(self) -> None:
        repository_path = f"/repos/{self.requested}"
        api = ScriptedApi(
            [
                (
                    ("GET", repository_path, None),
                    (200, exact_repository(self.actual)),
                )
            ]
        )
    ...
    def test_create_only_helper_rejects_cross_owner_or_public_redirect(self) -> None:
        ...
        for payload, message in (
            (redirect_repository("other-owner/sp-web-leptos"), "identity mismatch"),
            (redirect_repository(self.actual, visibility="public"), "visibility mismatch"),
        ):
    ...
    def test_create_conflict_does_not_reconcile_to_redirect_target(self) -> None:
        repository_path = f"/repos/{self.requested}"
        organization_path = "/orgs/streempilot/repos"
        redirect = redirect_repository(self.actual)
        api = ScriptedApi(
            [

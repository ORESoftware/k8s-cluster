...
unsafe archives, unverified workflow evidence, and secret-bearing candidate files.
"""
...
def verify_final(github: Github, candidate: Candidate) -> dict[str, Any]:
...
def run(request_path: Path, validate_only: bool, artifact_path: Path | None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="shared-auth-provision-") as directory:
        ...
        result = verify_final(github, candidate)

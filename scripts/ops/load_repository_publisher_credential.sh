...
# Prefer the repository-scoped Actions secret. It is already masked and avoids
# coupling repository publication to the permissions of an unrelated AWS OIDC
# maintenance role. Keep Secrets Manager as a recovery path for environments
# where the Actions secret is intentionally absent.
...
# Mask before emitting the credential into GitHub's environment file. The next
# step independently verifies the exact user and organization-admin boundary
# before any repository mutation is permitted.
printf '::add-mask::%s\n' "$token"
{
  printf 'GH_TOKEN=%s\n' "$token"

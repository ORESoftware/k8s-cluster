...
job="benefactor-provider-probe-v2-${short_sha}-$(date -u +%H%M%S)"
manifest="$(mktemp /tmp/benefactor-provider-probe-v2.XXXXXX.yaml)"
log_file="$(mktemp /tmp/benefactor-provider-probe-v2.XXXXXX.log)"
job_created=false
...
metadata:
  ...
  namespace: ${namespace}
  labels:
    app.kubernetes.io/name: benefactor-provider-probe-v2
    ...
    spec:
      ...
      initContainers:
        - name: fetch-rds-ca
          ...
          args:
            - |
              ...
              const response = await fetch('https://truststore.pki.rds.amazonaws.com/global/global-bundle.pem', {
                ...
                headers: { 'User-Agent': 'benefactor-provider-probe-v2/1' },
              });
          ...
          volumeMounts:
            - { name: rds-ca, mountPath: /run/secrets/rds }
      ...
      containers:
        - name: provider-probe
          ...
          args:
            - |
              ...
              const client = new pg.Client({
                ...
                query_timeout: 35000,
                application_name: 'benefactor-provider-probe-v2',
              });
          ...
          securityContext:
            ...
            seccompProfile: { type: RuntimeDefault }
          env:
            - { name: PIPELINE_DRY_RUN, value: "true" }
            - { name: TARGET_EMAILS, value: "1" }
            - { name: MAX_QUERIES, value: "1" }
            - { name: MAX_PAGES_PER_QUERY, value: "1" }
            - { name: DOMAIN_SKIP_DAYS, value: "0" }
            - { name: QUERY_COOLDOWN_DAYS, value: "0" }
            - { name: DEADLINE_SECONDS, value: "180" }
            - { name: SCRAPER_URL, value: http://dd-web-scraper.default.svc.cluster.local:8097 }
            - { name: SCRAPER_ALLOWED_HOSTS, value: dd-web-scraper.default.svc.cluster.local }
            - { name: REQUIRE_ROLE_EMAIL, value: "true" }
            - name: RDS_URL
              valueFrom: { secretKeyRef: { name: dd-remote-rest-api-secrets, key: AGENT_TASKS_RDS_DATABASE_URL } }
            - name: SCRAPER_AUTH
              valueFrom: { secretKeyRef: { name: dd-agent-secrets, key: SERVER_AUTH_SECRET } }
            - name: SERPER_API_KEY
              valueFrom: { secretKeyRef: { name: ${provider_secret}, key: SERPER_API_KEY, optional: true } }
            - name: BRAVE_SEARCH_API_KEY
              valueFrom: { secretKeyRef: { name: ${provider_secret}, key: BRAVE_SEARCH_API_KEY, optional: true } }
            - { name: PG_SSL_CA_FILE, value: /run/secrets/rds/global-bundle.pem }
          resources:
            requests: { cpu: 100m, memory: 256Mi }
            limits: { cpu: "1", memory: 1Gi }
          volumeMounts:
            - { name: rds-ca, mountPath: /run/secrets/rds, readOnly: true }
            - { name: tmp, mountPath: /tmp }
      volumes:
        - name: rds-ca
...
"$KUBECTL" --kubeconfig "$KUBECONFIG_PATH" -n "$namespace" logs "job/$job" -c provider-probe > "$log_file" 2>/dev/null || true

...
REST_AUTH=$($KX -n default get secret dd-agent-secrets -o jsonpath='{.data.SERVER_AUTH_SECRET}' 2>/dev/null | base64 -d 2>/dev/null)
if [ -z "$REST_AUTH" ]; then
  REST_AUTH=$($KX -n default get secret dd-remote-rest-api-secrets -o jsonpath='{.data.SERVER_AUTH_SECRET}' 2>/dev/null | base64 -d 2>/dev/null)
fi

curl_rest() {
  $KX exec -n default deploy/dd-remote-rest-api -- \
    curl -sf -H "X-Server-Auth: ${REST_AUTH}" "$@"
}

curl_rest_soft() {
  $KX exec -n default deploy/dd-remote-rest-api -- \
    curl -s -H "X-Server-Auth: ${REST_AUTH}" "$@"
}
...
note "2. REST API auth gate"
if [ -n "$REST_AUTH" ]; then
  ok "loaded server auth secret for authenticated in-cluster probes"
else
  ko "could not load SERVER_AUTH_SECRET from Kubernetes secrets"
fi
UNAUTH_CODE=$($KX exec -n default deploy/dd-remote-rest-api -- curl -s -o /dev/null -w '%{http_code}' "${REST}/api/container-pool/images" 2>/dev/null)
echo "    unauthenticated status: $UNAUTH_CODE"
[ "$UNAUTH_CODE" = "401" ] && ok "direct in-cluster API rejects missing X-Server-Auth" || ko "direct API returned $UNAUTH_CODE without auth"

note "3. REST API healthz"
...
note "4. GET /api/container-pool/images (list)"
LIST_JSON=$(curl_rest "${REST}/api/container-pool/images" 2>&1)
...
note "5. GET /api/container-pool/images/python3/dockerfile?source=disk-default"
DF=$(curl_rest "${REST}/api/container-pool/images/python3/dockerfile?source=disk-default" 2>&1)
...
note "6. Schema contract (tables created from pg-defs/migration, not app DDL)"
...
note "7. PUT /api/container-pool/images/python3/dockerfile (save revision)"
...
SAVE=$(curl_rest -X PUT -H 'content-type: application/json' -d "$BODY" "${REST}/api/container-pool/images/python3/dockerfile" 2>&1)
...
note "8. POST /api/container-pool/images/python3/build-test"
if [ -n "$REVID" ]; then
  BT_BODY='{"revisionId":"'"$REVID"'"}'
else
  BT_BODY='{}'
fi
BT=$(curl_rest_soft -X POST -H 'content-type: application/json' -d "$BT_BODY" "${REST}/api/container-pool/images/python3/build-test" 2>&1)
...
note "9. Poll build status (up to 6 min)"
OS=""
if [ -n "$BUILD_ID" ]; then
  for i in $(seq 1 72); do
    sleep 5
    POLL=$(curl_rest "${REST}/api/container-pool/builds/${BUILD_ID}" 2>&1)
  ...
  curl_rest "${REST}/api/container-pool/builds/${BUILD_ID}" 2>&1 | python3 -c '
import sys, json
...
note "10. GET /api/container-pool/images/python3/builds (history)"
HIST=$(curl_rest "${REST}/api/container-pool/images/python3/builds" 2>&1)
...
note "11. GET /api/container-pool/images/python3/revisions"
REV=$(curl_rest "${REST}/api/container-pool/images/python3/revisions" 2>&1)
...
note "12. Web page reachability /container-pool/config"
...
note "13. Candidate image landed in build namespace (k8s.io)"

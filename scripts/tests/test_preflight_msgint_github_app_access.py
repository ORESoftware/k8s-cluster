...
MODULE_PATH = pathlib.Path(__file__).resolve().parents[1] / "preflight-msgint-github-app-access.py"
...
class AccessPreflightTests(unittest.TestCase):
    def test_verifies_exact_private_repository_and_commit_without_token_in_urls(self):
        calls = []
        ...
        for request, timeout in calls:
            ...
            self.assertEqual(request.headers["X-github-api-version"], preflight.API_VERSION)
    ...
    def test_token_file_requires_absolute_private_regular_file(self):
        with tempfile.TemporaryDirectory() as temporary:
            directory = pathlib.Path(temporary)
            token_file = directory / "token"
            token_file.write_text(TOKEN, encoding="utf-8")
            token_file.chmod(0o600)
            self.assertEqual(preflight._read_token_file(str(token_file)), TOKEN)

            token_file.chmod(0o644)
            with self.assertRaisesRegex(preflight.PreflightError, "permissions"):
                preflight._read_token_file(str(token_file))

            token_file.chmod(0o600)
            with self.assertRaisesRegex(preflight.PreflightError, "absolute"):
                preflight._read_token_file("token")

            directory_path = directory / "not-a-token"
            directory_path.mkdir()
            with self.assertRaisesRegex(preflight.PreflightError, "regular file"):
                preflight._read_token_file(str(directory_path))

            symlink = directory / "token-link"
            symlink.symlink_to(token_file)
            with self.assertRaisesRegex(preflight.PreflightError, "regular file"):
                preflight._read_token_file(str(symlink))

    def test_http_denial_is_actionable_and_redacted(self):
    ...
    def test_rejects_repository_visibility_and_commit_substitution(self):
        def public_repository(request, timeout):
            return FakeResponse({"full_name": REPOSITORY, "private": False})

        with self.assertRaisesRegex(preflight.PreflightError, "identity or visibility"):
            preflight.verify_access(REPOSITORY, REVISION, TOKEN, opener=public_repository)

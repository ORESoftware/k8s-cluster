...
function replaceOnce(source, before, after, label) {
  ...
  if (index < 0) throw new Error(`DEN-267 marker not found: ${label}`);
...
function insertInCase(source, caseName, needle, insertion) {
  const marker = `case '${caseName}': {`;
  const start = source.indexOf(marker);
  if (start < 0) throw new Error(`DEN-267 case not found: ${caseName}`);
  ...
  if (relative < 0) throw new Error(`DEN-267 insertion marker missing in ${caseName}`);
  if (block.indexOf(needle, relative + needle.length) >= 0) {
    throw new Error(`DEN-267 insertion marker ambiguous in ${caseName}`);
  }
...
agent = replaceOnce(
  ...
  "            : e.code === 'domain_not_allowed'\n              ? 'domain_not_allowed'\n              : 'other';",
  "            : e.code === 'domain_not_allowed'\n              ? 'domain_not_allowed'\n              : e.code === 'sensitive_field_blocked'\n                ? 'sensitive_field_blocked'\n                : 'other';",
  'blocked error mapping',
);
await writeFile(agentPath, agent);

const dockerPath = 'remote/deployments/web-scraper-service/Dockerfile';
...
readme = replaceOnce(
  ...
  'runbook safety statement',
);
await writeFile(readmePath, readme);

console.log('DEN-267 current-main runtime integration applied');

<?php

// Generated by @dd/pg-defs. Do not edit by hand.
// SOURCE OF TRUTH: schema/schema.sql defines the database contract.
// Generated ORM/client code is an adapter only; do not infer migrations from it.
// MIGRATION SAFETY: never run or apply migrations automatically. Require explicit human review and approval before any database write.

declare(strict_types=1);

namespace DdPgDefs\Doctrine;

use Doctrine\ORM\Mapping as ORM;

#[ORM\Entity]
#[ORM\Table(name: 'billing_subscriptions', schema: 'fiducia')]
class BillingSubscriptions
{
    #[ORM\Id]
    #[ORM\GeneratedValue(strategy: 'NONE')]
    #[ORM\Column(type: 'guid')]
    public string $id;

    #[ORM\Column(type: 'guid', name: 'org_id')]
    public string $orgId;

    #[ORM\Column(type: 'guid', name: 'billing_customer_id')]
    public string $billingCustomerId;

    #[ORM\Column(type: 'string', length: 16)]
    public string $provider;

    #[ORM\Column(type: 'string', length: 255, name: 'provider_subscription_id')]
    public string $providerSubscriptionId;

    #[ORM\Column(type: 'string', length: 120)]
    public string $plan;

    #[ORM\Column(type: 'string', length: 32)]
    public string $status;

    #[ORM\Column(type: 'datetimetz_immutable', name: 'current_period_start', nullable: true)]
    public ?\DateTimeImmutable $currentPeriodStart;

    #[ORM\Column(type: 'datetimetz_immutable', name: 'current_period_end', nullable: true)]
    public ?\DateTimeImmutable $currentPeriodEnd;

    #[ORM\Column(type: 'boolean', name: 'cancel_at_period_end')]
    public bool $cancelAtPeriodEnd;

    #[ORM\Column(type: 'datetimetz_immutable', name: 'canceled_at', nullable: true)]
    public ?\DateTimeImmutable $canceledAt;

    #[ORM\Column(type: 'datetimetz_immutable', name: 'created_at')]
    public \DateTimeImmutable $createdAt;

    #[ORM\Column(type: 'datetimetz_immutable', name: 'updated_at')]
    public \DateTimeImmutable $updatedAt;
}

<?php

// Generated by @dd/pg-defs. Do not edit by hand.
// SOURCE OF TRUTH: schema/schema.sql defines the database contract.
// Generated ORM/client code is an adapter only; do not infer migrations from it.
// MIGRATION SAFETY: never run or apply migrations automatically. Require explicit human review and approval before any database write.

declare(strict_types=1);

namespace DdPgDefs\Doctrine;

use Doctrine\ORM\Mapping as ORM;

#[ORM\Entity]
#[ORM\Table(name: 'translations', schema: 't2v')]
class Translations
{
    #[ORM\Id]
    #[ORM\GeneratedValue(strategy: 'NONE')]
    #[ORM\Column(type: 'guid')]
    public string $id;

    #[ORM\Column(type: 'text', name: 'source_text')]
    public string $sourceText;

    #[ORM\Column(type: 'text', name: 'translated_text')]
    public string $translatedText;

    #[ORM\Column(type: 'text', name: 'source_lang', nullable: true)]
    public ?string $sourceLang;

    #[ORM\Column(type: 'text', name: 'target_lang')]
    public string $targetLang;

    #[ORM\Column(type: 'text')]
    public string $provider;

    #[ORM\Column(type: 'text')]
    public string $model;

    #[ORM\Column(type: 'bigint', name: 'latency_ms')]
    public string $latencyMs;

    #[ORM\Column(type: 'datetimetz_immutable', name: 'created_at')]
    public \DateTimeImmutable $createdAt;
}

<?php

// Generated by @dd/pg-defs. Do not edit by hand.
// SOURCE OF TRUTH: schema/schema.sql defines the database contract.
// Generated ORM/client code is an adapter only; do not infer migrations from it.
// MIGRATION SAFETY: never run or apply migrations automatically. Require explicit human review and approval before any database write.

declare(strict_types=1);

namespace DdPgDefs\Doctrine;

use Doctrine\ORM\Mapping as ORM;

#[ORM\Entity]
#[ORM\Table(name: 'des_fel_elevator_policy_states')]
class DesFelElevatorPolicyStates
{
    #[ORM\Id]
    #[ORM\GeneratedValue(strategy: 'NONE')]
    #[ORM\Column(type: 'guid')]
    public string $id;

    #[ORM\Column(type: 'guid', name: 'run_id')]
    public string $runId;

    #[ORM\Column(type: 'string', length: 40, name: 'policy_kind')]
    public string $policyKind;

    #[ORM\Column(type: 'string', length: 40, name: 'source_kind')]
    public string $sourceKind;

    #[ORM\Column(type: 'integer', name: 'feature_dim')]
    public int $featureDim;

    #[ORM\Column(type: 'integer', name: 'output_dim')]
    public int $outputDim;

    #[ORM\Column(type: 'integer', name: 'parameter_count')]
    public int $parameterCount;

    #[ORM\Column(type: 'bigint', name: 'online_learning_updates')]
    public string $onlineLearningUpdates;

    #[ORM\Column(type: 'json', name: 'loss_history')]
    public array $lossHistory;

    #[ORM\Column(type: 'json')]
    public array $state;

    #[ORM\Column(type: 'datetimetz_immutable', name: 'created_at')]
    public \DateTimeImmutable $createdAt;
}

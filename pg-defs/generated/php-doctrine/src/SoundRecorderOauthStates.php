<?php

// Generated by @dd/pg-defs. Do not edit by hand.
// SOURCE OF TRUTH: schema/schema.sql defines the database contract.
// Generated ORM/client code is an adapter only; do not infer migrations from it.
// MIGRATION SAFETY: never run or apply migrations automatically. Require explicit human review and approval before any database write.

declare(strict_types=1);

namespace DdPgDefs\Doctrine;

use Doctrine\ORM\Mapping as ORM;

#[ORM\Entity]
#[ORM\Table(name: 'sound_recorder_oauth_states')]
class SoundRecorderOauthStates
{
    #[ORM\Id]
    #[ORM\GeneratedValue(strategy: 'NONE')]
    #[ORM\Column(type: 'guid')]
    public string $id;

    #[ORM\Column(type: 'guid', name: 'account_id')]
    public string $accountId;

    #[ORM\Column(type: 'guid', name: 'device_id')]
    public string $deviceId;

    #[ORM\Column(type: 'string', length: 32)]
    public string $provider;

    #[ORM\Column(type: 'string', length: 64, name: 'state_hash')]
    public string $stateHash;

    #[ORM\Column(type: 'string', length: 512, name: 'redirect_uri')]
    public string $redirectUri;

    #[ORM\Column(type: 'string', length: 512, name: 'folder_path', nullable: true)]
    public ?string $folderPath;

    #[ORM\Column(type: 'string', length: 32)]
    public string $status;

    #[ORM\Column(type: 'datetimetz_immutable', name: 'expires_at')]
    public \DateTimeImmutable $expiresAt;

    #[ORM\Column(type: 'datetimetz_immutable', name: 'consumed_at', nullable: true)]
    public ?\DateTimeImmutable $consumedAt;

    #[ORM\Column(type: 'json', name: 'meta_data')]
    public array $metaData;

    #[ORM\Column(type: 'datetimetz_immutable', name: 'created_at')]
    public \DateTimeImmutable $createdAt;

    #[ORM\Column(type: 'datetimetz_immutable', name: 'updated_at')]
    public \DateTimeImmutable $updatedAt;
}

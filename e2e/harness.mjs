...
// Set AKRION_BACKEND_TEST_URL to run against an already-running server.
export async function startServer() {
  const reuse = process.env.AKRION_BACKEND_TEST_URL;
  if (reuse) return { url: reuse.replace(/\/+$/, ""), stop: () => {} };

  const port = await freePort();
  const url = `http://127.0.0.1:${port}`;

  const prebuilt = path.join(repoRoot, "target", "debug", "dd-soccer-rs");

...
| `quaestor-ledger` | [`quaestor-web-server.rs`](https://github.com/quaestor-ledger/quaestor-web-server.rs/blob/main/docs/web-api-data-access.md) | Combined API-owned database today; P2 on a physical split; P1 only through a distinct read-only role; browser wakeups are not P3 |
...
| `fiducia-cloud` | [`fiducia-customer.rs`](https://github.com/fiducia-cloud/fiducia-customer.rs/blob/main/docs/web-api-data-access.md) | Combined customer BFF/API database today; P2 for `fiducia-auth` and a future split; P1 only through a distinct shared read-only role; brain/node excluded |
| `canonical-cloud` | [`canonical-web-server.rs`](https://github.com/canonical-cloud/canonical-web-server.rs/blob/main/docs/web-api-data-access.md) | Combined local API/session authority today; dedicated quote API uses P2; future P1 requires a distinct read-only role; current wakeups are not P3 |
...
| `file-tunnel` | [`ftnl-web-server.rs`](https://github.com/file-tunnel/ftnl-web-server.rs/blob/main/docs/web-api-data-access.md) | P2 from the static portal to the control API; XHR upload progress is still bounded HTTP, not P3; the portal has no DB credential |
| `embedded-alerts` | [`eal-mash-web`](https://github.com/embedded-alerts/eal-mash-web/blob/main/docs/web-api-data-access.md) | Scaffold today; P1 only after tenant-safe read hardening, otherwise P2; measured P3 invalidations and durable P4 delivery jobs are production targets |
| `evento-globolo` | [`evgl-mash-web`](https://github.com/evento-globolo/evgl-mash-web/blob/main/docs/web-api-data-access.md) | Scaffold today; P1 only after public/tenant-safe read hardening, P2 for commands; measured P3 invalidations and P4 import/cross-post jobs are production targets |

...
1. GitHub Actions in both Rust repositories must finish and publish the `main` images to GHCR.
...
4. The Postgres schemas in each application's `db/schema.sql` must be applied through the cluster's normal migration process.

# Formal review procedure: audio-segment upload, verification, retention, mirroring, and erasure
...
The procedure covers segment metadata lifecycle and deletion safety; S3 consistency, cryptographic primitives, PostgreSQL transactions, and external cloud providers remain separately tested assumptions.
...
1. **SEG_VERIFY (Safety).** Client completion alone never marks a segment uploaded; authenticated object verification is required.
2. **SEG_PIN (Safety).** Pinned segments are never deleted by rolling retention.
3. **SEG_ERASE (Safety).** Erasure is final only after primary and known mirror objects are removed or proven absent.
4. **SEG_TRANSITION (Safety).** Only legal segment lifecycle transitions are reachable.
5. **SEG_RETRY (Refinement).** Retried presign, complete, mirror, and retention operations are idempotent or fail closed.
...
- mock-object-store tests for HeadObject, mirror, and delete failures
- database transaction tests for claims, pinning, retry, and account erasure

# Formal review procedure: refresh-session rotation, replay rejection, and identity safety
...
The procedure covers refresh-token issuance, atomic rotation, replay, logout, and revocation; cryptographic primitive correctness, PostgreSQL serializability, and provider JWKS transport are external assumptions that require separate tests and audits.
...
1. **AUTH_ROTATE (Safety).** A successful refresh consumes the presented token and installs one fresh current token.
2. **AUTH_REPLAY (Safety).** A consumed refresh token is rejected without changing session state.
3. **AUTH_REVOKE (Safety).** A revoked session accepts no refresh token.
4. **AUTH_IDENTITY (Safety).** External identities are keyed by authority/tenant/subject, never email equality alone.
5. **AUTH_CLAIMS (Refinement).** Issued and accepted JWTs preserve issuer, audience, algorithm, expiry, and session provenance constraints.
...
- transactional concurrent-refresh tests against disposable PostgreSQL
- provider identity collision and JWT claim-boundary tests

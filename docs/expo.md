...
The merge gate requires locked formatting, Clippy with warnings denied, all tests, the Rust 1.88 container build, cargo-deny, RustSec, and full-history Gitleaks to pass on the same reviewed commit. The final validation run uses the permanent read-only workflows without branch-mutating steps.

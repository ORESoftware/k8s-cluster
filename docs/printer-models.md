...
1. **Classification** — `machine_class()` resolves a model token (or alias) to
   its canonical `machine_kind`, so `"creality-k2-cfs"` classifies as additive/
...
| creality-k2 | Creality | fdm | klipper | CoreXY | 260×260×260 | 300 | 100 | 32 | yes | 1 |
| creality-k2-cfs | Creality | multi | klipper | CoreXY | 260×260×260 | 300 | 100 | 32 | yes | 16 |
...
- **Creality K2 / K2 with CFS** — the base K2 remains a single-material
  Klipper profile and advertises that it is CFS-ready. The separate
  `creality-k2-cfs` profile represents a K2 with the optional Creality Filament
  System installed, including chained configurations with up to 16 spools.

...
Validate the documents, SLO catalog, and gate matrix without installing dependencies:

```bash
node tools/validate-production-gates.mjs
```

A release candidate is not launchable until every SLO source is `measured` with exact-candidate evidence, every required gate row is `passed`, and this stricter command succeeds:

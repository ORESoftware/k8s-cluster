...
pub async fn api_docs() -> Html<String> {
    Html(
        r#"<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>shared-auth API</title><style>body{font:15px/1.5 ui-monospace,monospace;max-width:52rem;margin:3rem auto;padding:0 1rem}code{background:#8882;padding:.1rem .3rem}li{margin:.5rem 0}</style></head><body><h1>shared-auth API</h1><p>Postgres-primary, provider-neutral authentication. The machine-readable OpenAPI contract is at <a href="/api/docs.json"><code>/api/docs.json</code></a>.</p><ul><li><code>POST /auth/register</code> — optional local registration</li><li><code>POST /auth/login</code> — local login</li><li><code>POST /auth/exchange</code> — exchange a secondary-provider token</li><li><code>POST /auth/refresh</code> — rotate a refresh token</li><li><code>POST /auth/logout</code> — revoke a refresh session</li><li><code>POST /auth/introspect</code> — inspect a shared-auth access token</li><li><code>GET /.well-known/jwks.json</code> — public ES256 keys</li></ul></body></html>"#.to_owned(),
    )
}
...
fn document() -> Value {
    json!({
        ...
        "info": {
            ...
            "description": "Postgres-primary authentication with provider adapters, rotated sessions, and ES256 access tokens."
        },
        "paths": {

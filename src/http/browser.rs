...
use crate::{
    db::AuthenticatedIdentity, error::AuthError, state::AppState, token::AuthenticationAssurance,
    views,
};
...
pub async fn consume_otp(
    ...
    let identity = passwordless::consume_email_otp_identity(&state, &form.email, &form.otp).await?;
    issue_browser_session(&state, &config, &headers, identity, &return_state.return_to).await
}
...
fn remembered_emails(headers: &HeaderMap, config: &BrowserConfig) -> Vec<String> {
    ...
    let Ok(mut emails) = open_json::<Vec<String>>(&config.seal_secret, EMAILS_AAD, &cookie) else {
        return Vec::new();
    };
...
fn seal_json<T: Serialize>(secret: &str, aad: &[u8], value: &T) -> Result<String, AuthError> {
    ...
    let cipher =
        <Aes256Gcm as AeadKeyInit>::new_from_slice(&key).map_err(|_| AuthError::Internal)?;
...
fn open_json<T: DeserializeOwned>(secret: &str, aad: &[u8], sealed: &str) -> Result<T, AuthError> {
    if sealed.len() > 4_096 {
        return Err(AuthError::Unauthorized);
    }
    ...
    let nonce_bytes: [u8; 12] = raw[..12].try_into().map_err(|_| AuthError::Unauthorized)?;
    let nonce = Nonce::from(nonce_bytes);
    let key = Sha256::digest(secret.as_bytes());
    let cipher =
        <Aes256Gcm as AeadKeyInit>::new_from_slice(&key).map_err(|_| AuthError::Internal)?;

...
fn authorize_delegation(
    ...
    let scopes =
        normalized_unique(&request.scopes).ok_or(AuthError::BadRequest("invalid scopes"))?;
    let policy = policies
        .iter()
        .find(|policy| policy.client_id == request.client_id && policy.audience == request.audience)
...
fn load_delegation_policies() -> Result<Vec<DelegationPolicy>, String> {
    ...
    if policies.is_empty() {
        tracing::warn!("AUTH_DELEGATION_POLICIES is empty; /auth/delegate will deny every request");
    }
    Ok(policies)
}

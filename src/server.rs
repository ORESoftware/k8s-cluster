...
use crate::{config, connector, crypto, forward, http_connect, relay, socks, stats, web};
...
pub(crate) async fn run() -> Result<()> {
    ...
    let _telemetry = fiducia_telemetry::init(telemetry_service(&role));
...
fn resolve_role(cli_role: Option<String>, environment_role: Option<String>) -> String {
    cli_role.or(environment_role).unwrap_or_default()
}

fn telemetry_service(role: &str) -> &'static str {
    match role {
        "client" => "tor-client",
        "relay" => "tor-relay",
        "keygen" => "tor-keygen",
...
async fn run_client() -> Result<()> {
    ...
    let forward_routes = match std::env::var("TOR_FORWARD")
        .ok()
        .filter(|value| !value.is_empty())
    {
    ...
    let directory = match std::env::var("TOR_DIRECTORY") {
        Ok(path) => Some(config::Directory::load(&PathBuf::from(&path))?),
        Err(_) => None,
    };
    ...
    for forward_cfg in forward_cfgs {
        tasks.spawn(async move { forward::run(forward_cfg).await });
    }
    if let Some(result) = tasks.join_next().await {
        return result.context("client task panicked")?;
    }
    Ok(())
}
...
#[cfg(test)]
mod tests {
    use super::{is_loopback_listener, resolve_role, telemetry_service};
    ...
    #[test]
    fn cli_role_precedes_environment_role() {
        assert_eq!(
            resolve_role(Some("client".to_string()), Some("relay".to_string())),
            "client"
        );
        assert_eq!(resolve_role(None, Some("relay".to_string())), "relay");
        assert_eq!(resolve_role(None, None), "");
    }

    #[test]
    fn telemetry_names_are_role_specific_and_safe_for_unknown_commands() {
        assert_eq!(telemetry_service("client"), "tor-client");
        assert_eq!(telemetry_service("relay"), "tor-relay");
        assert_eq!(telemetry_service("keygen"), "tor-keygen");
        assert_eq!(telemetry_service("unexpected"), "tor-server");
    }
}

...
//! acquire path is INSERT ... ON CONFLICT DO UPDATE whose conditional-preempt
//! semantics the entity API cannot express, and the audit inserts cast into
//! the `lock_event_kind` enum. Transaction boundaries are identical to the
//! sqlx implementation this was converted from.
...
impl LockService {
    pub fn new(pool: DatabaseConnection, fiducia: FiduciaCoordinator) -> Self {
        Self { pool, fiducia }
    }
    ...
    pub async fn renew(
        ...
        let local_exists: bool = require_row(
            self.pool
                .query_one(stmt(
                    r#"
            SELECT EXISTS(
    ...
    async fn persist_acquire(
        ...
        advisory_lock(&tx, tenant_id, &req.resource).await?;
    ...
    async fn persist_renew(
        ...
        let shard = ShardKey::derive(tenant_id, region).0;
        let tx = self.pool.begin().await?;
        advisory_lock(&tx, tenant_id, resource).await?;
    ...
    async fn persist_release(
        ...
        let deleted = tx
            ...
            WHERE tenant_id = $1 AND resource_key = $2 AND lease_token = $3
            "#,
                [tenant_id.into(), resource.into(), lease_token.into()],
            ))
            .await?
            .rows_affected();

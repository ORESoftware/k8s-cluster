...
impl RealmConfig {
    ...
    fn validate(input: RealmInput, provider_refs: &[&str]) -> Result<Self, ConfigError> {
        ...
        validate_reference(&input.database_resource_ref, "AUTH_DATABASE_RESOURCE_REF")?;
        ...
        if !input.database_secret_ref.contains(&format!("/{realm}/")) {
            return Err(ConfigError::Invalid(
        ...
        let provider_matches =
            provider_refs.len() == 1 && provider_refs[0] == input.supabase_project_ref.as_str();
...
fn validate_issuer(value: &str, realm: Realm, allow_loopback: bool) -> Result<bool, ConfigError> {
...
mod tests {
    ...
    fn requires_exactly_one_matching_supabase_project() {
        ...
        let candidate = input(Realm::Admin);
        assert!(
            RealmConfig::validate(candidate, &["adminprojectref01", "legacyprojectref01"],)
                .is_err()
        );
    }
    ...
    fn loopback_requires_an_explicit_escape_hatch() {
        ...
        candidate.database_url = "postgres://runtime:secret@127.0.0.1:5432/shared_auth".to_owned();

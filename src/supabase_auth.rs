...
impl SupabaseVerifier {
    ...
    async fn refresh_jwks(&self) -> Result<(), AuthError> {
        let jwks_url = self
            ...
            .jwks_url
            .as_deref()
            .ok_or_else(|| AuthError::Unavailable("Supabase JWKS URL is not configured".into()))?;
...
mod tests {
    ...
    async fn concurrent_cold_misses_are_single_flighted() {
        ...
        for h in handles {
            h.await.unwrap().expect("all concurrent verifications succeed");
        }
        assert_eq!(
            server.hits.load(Ordering::SeqCst),
            1,
    ...
    async fn hs256_is_accepted_only_when_the_legacy_secret_is_configured() {
        ...
        let token =
            jsonwebtoken::encode(&header, &claims, &EncodingKey::from_secret(b"legacy-secret"))
                .unwrap();
        ...
        let with =
            SupabaseVerifier::from_config(&config_for(None, Some("legacy-secret".into())))
                .expect("configured");
    ...
    fn tenant_ids_are_read_from_app_metadata_only() {
        ...
        .unwrap();
        assert_eq!(claims.tenant_ids(), vec![Uuid::parse_str(TENANT_A).unwrap()]);
    }

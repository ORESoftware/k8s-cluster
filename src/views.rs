//! Server-rendered UI (Maud + htmx). The auth API is JSON; these are the
//! human-facing HTML blobs: a status landing page and a token-exchange helper
//! whose form posts to an htmx endpoint that swaps a result fragment in place.
...
/// Page layout. htmx is loaded from the CDN (internal tool); if unavailable the
/// form still submits as a normal POST.
...
pub fn landing(project_count: usize, issuer: &str, db_enabled: bool) -> Markup {
    page(
        ...
        html! {
            ...
            p class="muted" {
                "Centralized OreSoftware auth. Verifies Supabase tokens across "
                (project_count) " project(s), mirrors identities into RDS, and mints "
                "unified OreSoftware JWTs."
            }
            h2 { "status" }
            table {
                tr { td { "issuer" } td { code { (issuer) } } }
                tr { td { "supabase projects" } td { (project_count) } }
                tr { td { "identity mirror" } td { @if db_enabled { span class="ok" { "enabled" } } @else { span class="muted" { "disabled" } } } }
            }
            h2 { "endpoints" }
            table {
...
/// GET /ui — a helper that posts a Supabase token to /ui/exchange via htmx.
pub fn sign_in() -> Markup {
    page(
        ...
        html! {
            ...
            form hx-post="ui/exchange" hx-target="#result" hx-swap="innerHTML" method="post" action="ui/exchange" {
...
/// htmx fragment: the successful exchange result.
...
/// htmx fragment: a failure (uniform, no detail).

//! Direct Prometheus exposition for the customer-facing Fiducia backend.
//!
//! OTLP export remains enabled through `fiducia-telemetry`; this module supplies
//! the explicit in-cluster `/metrics` scrape contract. Labels are intentionally
//! limited to a bounded HTTP method set and numeric response status. Customer,
//! organization, session, credential, and raw-path values must never enter this
//! registry.
...
const PREFIX: &str = "fiducia_backend";
...
/// Prometheus text exposition endpoint. It is deliberately authentication-free
/// and is expected to remain reachable only through the in-cluster Service.
pub(crate) async fn endpoint() -> impl IntoResponse {
    ([(header::CONTENT_TYPE, CONTENT_TYPE)], render())
}

/// Record low-cardinality HTTP RED signals without duplicating OTLP export.
pub(crate) async fn record_http(request: Request<Body>, next: Next) -> Response {
    // Do not make Prometheus scrapes mutate the application request series.
...
mod tests {
    ...
    #[tokio::test]
    async fn endpoint_uses_prometheus_content_type() {
    ...
    #[test]
    fn render_contains_declared_low_cardinality_request_metrics() {
        record("GET", 200, Duration::from_millis(125));
        let body = render();

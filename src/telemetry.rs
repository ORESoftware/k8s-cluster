...
const DEFAULT_FILTER: &str = "info,dd_soccer_rs=info,tower_http=info";
...
impl TelemetryConfig {
    fn from_env() -> Self {
        ...
        let traces_enabled = env_bool("AKRION_OTEL_TRACES", endpoint_configured);
        let metrics_enabled = env_bool("AKRION_OTEL_METRICS", endpoint_configured);
        ...
        Self {
            ...
            service_name: env_string("AKRION_SERVICE_NAME")
                .unwrap_or_else(|| "dd-soccer-rs".to_string()),
...
pub(crate) fn init() -> TelemetryGuard {
    ...
    let tracer_provider = config.traces_endpoint.as_deref().and_then(|endpoint| {
        build_tracer_provider(endpoint, resource.clone())
            .map_err(|err| {
                eprintln!(
                    "akrion_telemetry_trace_init_failed service={} error={err}",
                    config.service_name
                );
            })
            .ok()
    ...
    let meter_provider = config.metrics_endpoint.as_deref().and_then(|endpoint| {
        build_meter_provider(endpoint, resource)
            .map_err(|err| {
                eprintln!(
                    "akrion_telemetry_metric_init_failed service={} error={err}",
                    config.service_name
                );
            })
            .ok()
    ...
    if let Err(err) = init_result {
        eprintln!(
            "akrion_telemetry_subscriber_init_failed service={} error={err}",
            config.service_name
        );
    }
...
fn http_metrics() -> &'static HttpMetrics {
    ...
    METRICS.get_or_init(|| {
        let meter = global::meter("dd-soccer-rs/http");
        HttpMetrics {
            requests: meter

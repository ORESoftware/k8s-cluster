...
- [`sonus-arc-github.externalsecret.template.yaml`](sonus-arc-github.externalsecret.template.yaml):
  AWS Secrets Manager to Kubernetes Secret field mapping; contains names only,
  never values.
- [`sonus-ci-smoke.workflow.template.yml`](sonus-ci-smoke.workflow.template.yml):
  manual, read-only first-registration proof for a trusted Sonus repository.
- [`../../../deployments/sonus-auris-ci-runner/Dockerfile`](../../../deployments/sonus-auris-ci-runner/Dockerfile):
  pinned ARC runner base plus Java 17 and Linux compiler, desktop, and browser
...
installing a second controller. The existing controller and runner-scale-set
chart versions must remain identical.
...
3. Reconcile the GitHub App secret through the reviewed ExternalSecret mapping.
...
5. Copy reviewed activation resources into `remote/argocd/apps/` only after all
   placeholders are replaced. Keep the source templates inert.
6. Confirm the scale set appears in **Organization Settings → Actions → Runners**.
7. Copy the smoke template to one trusted first-party repository and run it by
   `workflow_dispatch`; it must perform no external writes and print no secrets.
...
- Non-root runner container, dropped capabilities, seccomp RuntimeDefault,
  bounded CPU/memory/ephemeral storage, no hostPath mounts, and no service-account

...
def policy_json(text: str) -> dict[str, object]:
    match = re.search(r"(?ms)^\s*policy\.json:\s*\|-\s*\n(?P<body>(?:\s{4,}.*\n?)*)$", text)
    require(match is not None, "policy ConfigMap is missing policy.json block")
...
def check_files() -> None:
    for relative in (
        ...
        "streempilot-ci-runner-set.application.template.yaml",
        "streempilot-arc-github.externalsecret.template.yaml",
        "streempilot-ci-smoke.workflow.template.yml",
        "aws.applications.template.yaml",
        "hetzner.applications.template.yaml",
    ):
        read(BASE / relative)
...
def check_inert_boundary() -> None:
    ...
    for forbidden in (
        ...
        require(forbidden not in kustomization, f"active base references inert template {forbidden}")

    for cloud in ("aws", "hetzner"):
        text = read(BASE / f"{cloud}.applications.template.yaml")
...
def check_three_apps() -> None:
    ...
    reject_tokens(broker, ("GITHUB_TOKEN", "billing_token", "personal_access_token"), "broker")
...
def check_templates_and_smoke() -> None:
    runner_template = read(BASE / "streempilot-ci-runner-set.application.template.yaml")
    require_tokens(
        runner_template,
        (
...
def check_docs_and_credentials() -> None:
    ...
    combined = "\n".join(path.read_text(encoding="utf-8") for path in BASE.rglob("*.*") if path.is_file())
    reject_tokens(
        combined,
        (
            "ghp_",

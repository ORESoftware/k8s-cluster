# `remote/gleamlang-ws-server`

Unified Gleam + OTP websocket service that merges the two predecessors:

- **`remote/gleamlang-server`** — the legacy "broadcaster" service: a
...
- **`remote/gleamlang-presence-server`** — the new presence cluster:

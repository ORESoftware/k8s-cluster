#!/usr/bin/env bash
        ...
        BRIDGE_TOKEN_VALUE="aaaaaaaaaaaaaaaa"
        ...
        matching="$(curl --silent --show-error \
          ...
          --header 'x-message-id: same' \
          --header 'idempotency-key: same' \
          --header 'nats-msg-id: same' \
          --header 'content-type: application/json' \
          --data '{"case":"matching"}')"
        python3 -c 'import json,sys; p=json.loads(sys.argv[1]); assert p["ok"] is True; assert p["durable"] is False; assert p["messageId"] == "same"' "$matching"

        [[ "$(status \
          --request POST "${BRIDGE_URL}/publish/nats.header.test.auth" \
          --header "authorization: Bearer ${BRIDGE_TOKEN_VALUE}" \
          --header 'x-bridge-token: bbbbbbbbbbbbbbbb' \
          --header 'content-type: application/json' \

...
fn locked_git_args(args: Vec<String>) -> Vec<String> {
    let mut command = vec![
        "-c".to_string(),
...
fn clone_repository_commands(request: &BuildRequest, repo_dir: &Path) -> Vec<Vec<String>> {
    ...
    match clean_optional(request.git_ref.as_deref()) {
        ...
        git_ref => {
            ...
            args.extend(["--".to_string(), request.repo_url.clone(), destination]);
...
mod tests {
    ...
    fn immutable_clone_plan_checks_out_the_requested_non_tip_commit() {
        ...
        run_git(
            &source,
            &["config", "user.email", "gha-clone@example.invalid"],
        );
        ...
        run_git(
            &root,
            &["clone", "--bare", "--", &source_text, &remote_text],
        );

...
function truncateRepoContextFile(value: string, maxChars: number): string {
  if (value.length <= maxChars) {
    return value;
  }
...
async function listMarkdownChildren(workspacePath: string, relativeDir: string): Promise<string[]> {
  try {
...
async function existingContextFiles(workspacePath: string, relativePaths: string[]): Promise<string[]> {
  const out: string[] = [];

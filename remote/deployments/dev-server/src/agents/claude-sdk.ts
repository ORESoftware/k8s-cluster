...
function isRunnableExecutable(executable: string): boolean {
  try {
...
function resolveSdkNativeExecutable(packageNames: string[]): string | undefined {
...
export const claudeSdkRunner: AgentRunner = {
  ...
  async run(opts: AgentRunOpts): Promise<void> {
    ...
    try {
      sdk = (await import('@anthropic-ai/claude-agent-sdk')) as ClaudeAgentSdkModule;
    } catch (err) {
      throw new Error(
        'claude-sdk runner: @anthropic-ai/claude-agent-sdk cannot be imported.\n' +

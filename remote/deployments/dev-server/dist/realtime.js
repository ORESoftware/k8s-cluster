...
function getClient() {
    ...
    try {
        ...
        clientRetries += 1;
        process.stderr.write(`[remote-dev realtime] client creation failed (attempt ${clientRetries}/${MAX_CLIENT_RETRIES}): ${err instanceof Error ? err.message : String(err)}\n`);
        if (clientRetries >= MAX_CLIENT_RETRIES) {
            process.stderr.write("[remote-dev realtime] max client retries reached; disabling Realtime\n");
        }
        return null;
    }
}
...
export function getRemoteDevUserChannelName(userId) {
...
function startHealthCheck() {
    ...
    healthTimer = setInterval(() => {
        ...
        for (const [userId, entry] of channelPool) {
            const stale = entry.ready &&
                entry.lastSendAt > 0 &&
                now - entry.lastSendAt > CHANNEL_STALE_MS;
            const unhealthy = entry.consecutiveFailures >= CHANNEL_FAILURE_THRESHOLD;
            if (stale || unhealthy) {
                process.stderr.write(`[remote-dev realtime] recycling channel for user ${userId.slice(0, 8)} (stale=${stale}, failures=${entry.consecutiveFailures})\n`);
                void removeChannel(userId, entry);
            }
        }
    }, CHANNEL_HEALTH_INTERVAL_MS);
}
async function removeChannel(userId, entry) {
...
function getOrCreateEntry(userId) {
...
async function sendOnChannel(entry, payload, senderId) {
...
export function acquireUserChannel(userId) {
...
export function releaseUserChannel(userId) {
...
export async function publishUserEvent(userId, payload, senderId) {
    const entry = getOrCreateEntry(userId);
    if (!entry) {
        return;
    }
    ...
    const publish = previous
        ...
        process.stderr.write(`[remote-dev realtime] publish failed: ${err instanceof Error ? err.message : String(err)}\n`);
...
export function destroyChannelPool() {
    if (healthTimer) {

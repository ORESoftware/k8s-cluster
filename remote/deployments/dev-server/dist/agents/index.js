...
import { genericAiSdkRunner, DEFAULT_GENERIC_AI_SDK_SOURCES, defaultGenericAiSdkModels, } from './generic-ai-sdk.js';
...
function configuredSecretList(name) {
...
function configuredGenericAiSdkKeys(sourceId) {
...
function genericAiSdkBaseUrl(sourceId, fallback) {
    if (sourceId === 'opencode') {
...
function genericAiSdkModels(sourceId) {
...
function configuredProviderApiKeys(provider) {
    ...
    if (provider === 'opencode-ai-sdk') {
        return uniqueSecrets(DEFAULT_GENERIC_AI_SDK_SOURCES.flatMap((source) => configuredGenericAiSdkKeys(source.id)));
    }
 ...
 */
export function resolveAgentProvider(perTaskOverride) {
    let chosen;
 ...
 */
export function buildAgentEnv(provider, apiKey) {
    const base = {
...
export function buildAgentEnvCandidates(provider) {
    if (provider === 'opencode-ai-sdk') {
        const sourceEntries = DEFAULT_GENERIC_AI_SDK_SOURCES.flatMap((source) => configuredGenericAiSdkKeys(source.id).map((key) => ({
            source,
            key,
        })));
...
export function configuredAgentCredentialCount(provider) {
 ...
 */
async function probePackage(pkg) {
    try {
        await import(pkg);
        return true;
    }
...
export async function probeAllProviders() {
    ...
    const [hasClaude, hasCodex, hasClaudeSdkPackage, hasGeminiSdk, hasOpenaiSdk, hasAiSdk, hasOpenaiCompatibleSdk,] = await Promise.all([
        probeBinary('claude'),
        probeBinary('codex'),

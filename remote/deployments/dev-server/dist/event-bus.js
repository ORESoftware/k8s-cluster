...
export class EventBus {
    ...
    _ingestCircuit$ = new BehaviorSubject({
        consecutiveFailures: 0,
        isOpen: false,
        lastFailAt: 0,
    });
    /** Public observable — downstream can subscribe to all events. */
    all$ = this._events$.asObservable().pipe(share());
    /** Expose circuit state for health monitoring. */
    ingestCircuitState$ = this._ingestCircuit$.asObservable();
    getCircuitState() {
     ...
     * route.
     */
    startVercelIngest(ingestUrl, secret) {
        ...
        filter(() => {
            ...
            return false;
        }), bufferTime(250, undefined, 10), filter((batch) => batch.length > 0), concatMap((batch) => {
            ...
            mergeMap(async (res) => {
                ...
                const next = {
                    ...
                    isOpen: state.consecutiveFailures + 1 >=
                        CIRCUIT_FAILURE_THRESHOLD,
                    lastFailAt: Date.now(),
                };
                this._ingestCircuit$.next(next);
                if (next.isOpen) {
                    process.stderr.write(`[event-bus] circuit OPEN after ${next.consecutiveFailures} consecutive failures — dropping events for ${CIRCUIT_RESET_MS / 1000}s\n`);
                }
                else {
                    process.stderr.write(`[event-bus] ingest failed after retries (${next.consecutiveFailures}/${CIRCUIT_FAILURE_THRESHOLD}): ${err instanceof Error ? err.message : String(err)}\n`);
                }
                return EMPTY;
            })), 3));
        }), map(() => undefined))
     ...
     */
    startSupabaseBroadcast(publishFn) {
        const sub = this._events$
            .pipe(takeUntil(this._destroy$), filter((ev) => !!ev.userId), mergeMap((ev) => from(publishFn(ev.userId, {
            taskId: ev.taskId,
            threadId: ev.threadId,
            seq: ev.seq,
            event: ev.event,
        })).pipe(timeout(5_000), retry({ count: 3, delay: 1000 }), catchError((err) => {
            process.stderr.write(`[event-bus] broadcast failed: ${err instanceof Error ? err.message : String(err)}\n`);
            return EMPTY;
        })), 3), map(() => undefined))
            .subscribe();
        this._subs.push(sub);
    ...
    startLogSink(logDir) {
        ...
        const sub = this._events$
            .pipe(takeUntil(this._destroy$), bufferTime(100, undefined, 20), filter((batch) => batch.length > 0), concatMap((batch) => {
            ...
            for (const ev of batch) {
                ...
                const kindDetail = ev.event.kind === "status"
                    ? ` status=${ev.event.status ?? "?"}`
                    : ev.event.kind === "error"
                        ? ` message=${String(ev.event.message ?? "").slice(0, 200)}`
                        : ev.event.kind === "done"
                            ? ` exitReason=${ev.event.exitReason ?? "?"}`
                            : "";
                lines.push(`[${ts}] task=${ev.taskId} seq=${ev.seq} kind=${ev.event.kind}${kindDetail}`);
            }
            const writes = Array.from(byThread.entries()).map(async ([threadKey, lines]) => {
                const dir = threadKey === "__default__"
                    ? logDir
     ...
     */
    startIdleWatchdog(idleMs, onIdle) {
        ...
        const watchdogSub = interval(60_000)
            .pipe(takeUntil(this._destroy$), filter(() => !fired && Date.now() - lastActivity > idleMs))
            .subscribe(() => {
            fired = true;
            process.stderr.write(`[event-bus] idle watchdog triggered after ${idleMs / 1000}s — signaling shutdown\n`);
            onIdle();
        });
     ...
     */
    sseStream$(taskId, afterSeq = -1) {
        return this.taskEvents$(taskId).pipe(filter((ev) => ev.seq > afterSeq));
    }
    // ---- Lifecycle ----
    /**
    ...
    destroy() {
        ...
        this._subs.length = 0;
        for (const [, replay] of this._taskReplays) {
            replay.complete();
        }

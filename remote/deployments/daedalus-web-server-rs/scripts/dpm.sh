...
#                         DAEDALUS_WEB_DATABASE_URL, DATABASE_URL,
...
target="${TARGET_DATABASE_URL:-${DAEDALUS_WEB_DATABASE_URL:-${DATABASE_URL:-${RDS_DATABASE_URL:-}}}}"
...
case "$cmd" in
  ...
  diff | verify | apply | review)
    if [ -z "$target" ]; then
      ...
      echo "DAEDALUS_WEB_DATABASE_URL, DATABASE_URL, RDS_DATABASE_URL)." >&2
      exit 1
    fi

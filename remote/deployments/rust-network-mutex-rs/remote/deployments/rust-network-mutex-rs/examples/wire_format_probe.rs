...
fn main() {
    ...
    println!("RESP: {}", serde_json::to_string(&resp).unwrap());

    let req2 = Request::RegisterRead { uuid: "u2".into(), key: "k".into() };
    println!("REGREAD: {}", serde_json::to_string(&req2).unwrap());
    ...
    let comp = Response::CompositeLock {
        ...
        fencing_tokens: Some(std::collections::BTreeMap::from([("a".into(), 1u64), ("b".into(), 2u64)])),
        error: None,
    };
    println!("COMPOSITE: {}", serde_json::to_string(&comp).unwrap());
}

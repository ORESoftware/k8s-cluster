...
/// — see upstream `live-mutex`'s `max` field). Operators raising this
/// past 100 should know what they're doing: every concurrent holder
/// occupies a row in the per-key holder map and its own deadline entry,
/// so the broker's per-key cost is proportional to `max`.
///
...
/// raised at startup with `LMX_MAX_CONCURRENCY_CAP`; cross-runtime
/// clients read the *current* effective cap from `/metrics` rather than
/// trusting a baked-in constant.
pub const DEFAULT_MAX_CONCURRENCY_CAP: u32 = 100;

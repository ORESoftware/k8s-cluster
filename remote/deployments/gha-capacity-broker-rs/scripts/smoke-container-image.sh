...
operator_env_name='SERVER_AUTH_''SECRET'
operator_env_value="$(printf '%s:%s:%s' 'capacity-broker-smoke' "$revision" "$run_suffix" | sha256sum | cut -d' ' -f1)"
...
docker run --detach --rm \
  ...
  --env "${operator_env_name}=${operator_env_value}" \

...
fn run_boids(
    ...
    let perception = request.perception.unwrap_or(8.0).max(0.1);
    let max_speed = request.max_speed.unwrap_or(2.0).max(0.01);
    let sep_w = request.separation_weight.unwrap_or(1.5).max(0.0);
    let align_w = request.alignment_weight.unwrap_or(1.0).max(0.0);
    let coh_w = request.cohesion_weight.unwrap_or(1.0).max(0.0);
    let mut rng = Rng::new(seed);
...
async fn run_nats_loop(state: AppState, subject: String, queue_group: String) {
    ...
    let mut subscription = match nats.queue_subscribe(subject, queue_group).await {
        ...
        Err(error) => {
            eprintln!("agent-sim nats subscribe failed: {error}");
            return;
        }
    };

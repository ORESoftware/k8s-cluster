...
(`remote/deployments/ws-loadtest-rs/` and `remote/deployments/gleamlang-ws-loadtest/`) gained a
...
docker run -d --name akkaws -p 8086:8086 \
    -v "$(pwd)"/remote/deployments/akka-ws-server:/work -w /work \
    maven:3.9.9-eclipse-temurin-21 java -jar target/dd-akka-ws-server.jar

# 2. Build the Rust loadtest once
docker run --rm -v "$(pwd)"/remote/deployments/ws-loadtest-rs:/work -w /work \
...
vars work for the Gleam loadtest (`remote/deployments/gleamlang-ws-loadtest/`).
...
`remote/deployments/ws-loadtest-rs/k8s/akkaws-{asyncjava,akkastreams}/` and
`remote/deployments/gleamlang-ws-loadtest/k8s/akkaws-{asyncjava,akkastreams}/`; the
...
```bash
cd remote/deployments/akka-ws-server
mvn -B test

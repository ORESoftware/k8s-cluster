...
`gha-clone-server` is a per-organization GitHub Actions capacity and routing broker. It is deliberately **not** a replacement implementation of GitHub's proprietary Actions control plane.
...
- The existing `dd-build-server` remains an independent bounded fallback for reviewed `run-profile` requests. `gha-clone-server` never accepts or executes repository-supplied shell commands.

//// Legacy broadcaster ticker, ported verbatim from the original
//// `gleamlang-server`. Maintains a 2-second tick stream over a list of
//// subscribed websocket subjects, deduping on JSON message ids, and
//// counts a small set of Prometheus-style metrics. Drives the
//// `/worker-ws/<secret>` ws path and the `/broadcast` HTTP endpoint
//// served by the merged `http_server.gleam`.
...
@external(erlang, "gleamlang_ws_server_ffi", "json_message_id")
fn json_message_id(payload: String) -> Result(String, Nil)

@external(erlang, "gleamlang_ws_server_ffi", "now_ms")
fn now_ms() -> Int

...
fn finish(
    ...
    SimResponse {
        ...
        estimate,
        standard_error: se,
        confidence_interval: [estimate - half, estimate + half],
...
fn run_option(
    ...
    let spot = require_positive(request.spot, "spot")?;
    let strike = require_positive(request.strike, "strike")?;
    let vol = require_positive(request.volatility, "volatility")?;
    let maturity = require_positive(request.maturity, "maturity")?;
...
fn run_queue(
    ...
    // Simulate `samples` customers through a single-server FIFO queue.
    let customers = samples;
    let mut clock = 0.0;
    let mut server_free_at = 0.0;
    let mut wait_acc = Accumulator::default();
    let mut area_l = 0.0; // time-integral of number in system
    let mut last_event = 0.0;
    let mut in_system: i64 = 0;
    // Approximate L by sampling number-in-system over arrivals; track departures
    // implicitly via server_free_at.
    let mut total_service = 0.0;
    for _ in 0..customers {
        let interarrival = rng.exponential(lambda);
        clock += interarrival;
        // Customers that have departed by `clock` reduce in_system. We model a
        // single server, so the queue length is captured through wait times; we
        // accumulate the time-average of `in_system` between arrivals.
        area_l += in_system as f64 * (clock - last_event);
        last_event = clock;
        ...
        wait_acc.push(wait);
        in_system += 1;
        // Drop customers whose departure precedes the next arrival on average:
        // approximate by decrementing once service completes before `clock`.
        if server_free_at <= clock {
            in_system = (in_system - 1).max(0);
        }
    }

    let mean_wait = wait_acc.mean;
    let mean_w = mean_wait + 1.0 / mu; // time in system = wait + service
    let l_estimate = if clock > 0.0 { area_l / clock } else { 0.0 };

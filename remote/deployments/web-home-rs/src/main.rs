...
static PATH_ROWS: &[PathRow] = &[
    ...
    PathRow { paths: &[PathEntry { label: "/des-rs/", href: Some("/des-rs/") }, PathEntry { label: "/des-rs/simulations", href: Some("/des-rs/simulations") }, PathEntry { label: "/des-rs/out/", href: Some("/des-rs/out/") }, PathEntry { label: "/des-rs/healthz", href: Some("/des-rs/healthz") }, PathEntry { label: "/des-rs/docs/api", href: Some("/des-rs/docs/api") }], target: "Rust DES engine (library) result pages", access: SERVER_AUTH, notes: "Landing page with per-simulation run buttons that execute the discrete-event-system.rs engine (git submodule) in-process and serve its rendered HTML/JSON result pages." },

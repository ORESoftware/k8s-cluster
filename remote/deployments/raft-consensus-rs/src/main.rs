    ...
    fn can_communicate(&self, a: usize, b: usize, tick: u64) -> bool {
        for partition in &self.partitions {
            if tick >= partition.from_tick && tick < partition.to_tick {
                let group_a = partition.groups.iter().position(|g| g.contains(&a));
                let group_b = partition.groups.iter().position(|g| g.contains(&b));
                // Nodes listed in different groups cannot talk. Nodes not listed
                // in any group are isolated from listed nodes during the window.
...
async fn simulate_http(State(state): State<AppState>, Json(request): Json<RaftRequest>) -> Response {
...
async fn run_nats_loop(state: AppState, subject: String, queue_group: String) {
    ...
    let mut subscription = match nats.queue_subscribe(subject, queue_group).await {
        ...
        Err(error) => {
            eprintln!("raft-consensus nats subscribe failed: {error}");
            return;
        }
    };

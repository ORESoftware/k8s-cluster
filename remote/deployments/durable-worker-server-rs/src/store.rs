...
impl NatsStateStore {
    ...
    pub async fn ensure(
        ...
        let config = kv::Config {
            ...
            description:
                "Durable worker run, step, worker, idempotency, signal, and lease-lane state"
                    .to_string(),
            history: 10,
        ...
        // async-nats 0.38 does not expose the later create-or-update helper.
        // Prefer the existing bucket, create when absent, and retry the lookup
        // once to tolerate several control-plane replicas starting together.
        let bucket = match jetstream.get_key_value(bucket_name).await {
            Ok(bucket) => bucket,
            Err(initial_get_error) => match jetstream.create_key_value(config).await {
                Ok(bucket) => bucket,
                Err(create_error) => {
                    jetstream
                        .get_key_value(bucket_name)
                        .await
                        .map_err(|retry_get_error| {
                            StoreError::Backend(format!(
                            "failed to get key-value bucket {bucket_name} ({initial_get_error}); \
                             failed to create it ({create_error}); failed to get it after a \
                             concurrent create ({retry_get_error})"
                        ))
                        })?
                }
            },
...
impl EventSink for MemoryEventSink {
    async fn publish(&self, event: &DurableEvent) -> Result<(), StoreError> {
        ...
        if events
            .iter()
            .all(|existing| existing.event_id != event.event_id)
        {

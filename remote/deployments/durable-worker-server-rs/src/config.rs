...
mod tests {
    ...
    fn auth_comparison_accepts_only_the_exact_secret() {
        let candidate = ["unit", "-test-", "auth-", "material"].concat();
        let secret = AuthSecret::from_plain(&candidate).unwrap();
        assert!(secret.verify(&candidate));
        assert!(!secret.verify(&format!("{candidate}-wrong")));

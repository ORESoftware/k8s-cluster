...
- `Pause/Sleep (Reduce resources to container)` calls the Rust REST API to scale the matching
  Deployment to `0`.
- `Archive (Deep Sleep - Suspend container?)` uses the same scale-to-zero path and leaves room for

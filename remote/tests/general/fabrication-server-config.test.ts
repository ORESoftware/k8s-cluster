...
test('rust fabrication server exposes planning, analysis, nats, and learning hooks', async () => {
  ...
  assert.match(
    readme,
    /accepted instruction\s+kinds including slicer, pellet-FGF, SLA\/resin, material-jetting, DED\/WAAM, composite-fiber, binder-jet, SLS\/powder, metal-PBF, mill-turn, laser\/waterjet\/plasma,\s+wire-EDM, and sinker-EDM job sheets/,
  );
  ...
  assert.match(readme, /mill-turn\/swiss G-code with C\/Y-axis live-tooling and subspindle transfer/);
  ...
  assert.match(
    readme,
    /slicer, pellet-FGF, SLA\/resin, material-jetting, DED\/WAAM, composite-fiber, binder-jet, SLS\/powder, metal-PBF, mill-turn, laser\/waterjet\/plasma,\s+wire-EDM, and sinker-EDM job sheets/,
  );
  assert.match(
    readme,
  ...
  assert.match(
    readme,
    /printer\s+async-nozzle-wait state, async-bed-target re-wait state, nozzle-cooldown\/\s+reheat state, bed-cooldown\/re-wait state, stepper-idle\/re-home state,\s+additive inch-units\/slicer conversion state,\s+extrusion-mode\/reset state, post-mode-switch extrusion reset state,\s+negative-Z extrusion\/Z-offset probe state, bed-leveling\/mesh restore state,\s+filament lot\/dry-storage\s+conditioning evidence, material-capacity\/runout evidence,\s+extrusion calibration\/flow\/pressure-advance evidence,\s+firmware retraction\/recover settings evidence,\s+high-speed input-shaper\/acceleration\/volumetric-flow evidence,\s+chamber\/enclosure\/thermal-soak evidence for warp-prone filament,\s+bed-adhesion, first-layer, fan-timing/,
  );
  assert.match(
    readme,

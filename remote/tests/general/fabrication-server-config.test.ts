...
assert.match(source, /sheet-forming-evidence-missing/);
  ...
  assert.match(
    source,
    /struct FabricationPlanResponse[\s\S]*simulation: SimulationReport[\s\S]*instruction_intent_map: InstructionIntentMap[\s\S]*improvements: Vec<InstructionImprovement>[\s\S]*improved_programs: Vec<ImprovedInstructionProgram>/,
  );
  assert.match(source, /manufacturing_handoff: ManufacturingHandoff/);
  assert.match(source, /design_package: DesignPackage/);
  assert.match(source, /quality_plan: QualityPlan/);
  ...
  assert.match(readme, /tool, offset, tool-life, support-media, artifact/);
  assert.match(readme, /change tools, split setups, refresh offsets/);
  ...
  assert.match(readme, /simulator, check, finding, boundary, recommended-action, artifact, reward/);
  assert.match(readme, /`POST \/fabrication\/learning\/outcomes`/);
  ...
  assert.match(readme, /dd\.fabrication\.learning-model-result-review\.v1/);
  assert.match(readme, /metric-failure counts, blocker\s+hints, and artifact hints/);
  assert.match(readme, /replay verification, metric\s+review, and\s+cleared promotion blockers/);
  assert.match(readme, /dd\.fabrication\.learning-optimizer-result-review\.v1/);

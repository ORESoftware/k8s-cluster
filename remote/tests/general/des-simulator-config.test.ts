...
test('rust discrete event simulator declares, validates, and runs async DES jobs', async () => {
  ...
  assert.match(source, /fn fibonacci_example_request\(\) -> Value/);
  assert.match(source, /fn temperature_control_example_request\(\) -> Value/);
  assert.match(source, /const MAX_ACTIVE_JOBS: u64 = 8/);
  ...
  assert.match(source, /StatusCode::TOO_MANY_REQUESTS/);
  ...
  assert.match(source, /\.route\("\/model\/examples", get\(examples_http\)\)/);
  assert.match(source, /\.route\("\/model\/examples\/:name", get\(named_example_http\)\)/);
  ...
  assert.match(readme, /GET \/model\/examples/);
  assert.match(readme, /fibonacci/);
  assert.match(readme, /temperature-control/);
  ...
  assert.match(deployment, /CARGO_HOME[\s\S]*value:\s*\/tmp\/cargo/);
  assert.match(deployment, /CARGO_TARGET_DIR[\s\S]*value:\s*\/tmp\/des-target/);
  ...
  assert.match(deployment, /name:\s*tmp[\s\S]*mountPath:\s*\/tmp/);

...
test('revocation credentials remain distinct, required, and cloud-backed', async () => {
  const externalSecret = await readRepoFile(
    'remote/argocd/fiducia/fiducia-revocation-secrets.externalsecret.yaml',
  );

  assert.match(externalSecret, /refreshInterval:\s*5m/);
  assert.match(externalSecret, /secretStoreRef:[\s\S]*kind:\s*ClusterSecretStore[\s\S]*name:\s*dd-fiducia-kv/);
  ...
  assert.equal(literalCount(externalSecret, 'secretKey: admin-secret'), 1);
  assert.equal(literalCount(externalSecret, 'secretKey: reader-secret'), 1);
  assert.doesNotMatch(
    externalSecret,
...
test('revocation authority is an immutable least-privilege runtime', async () => {
  ...
  assert.match(source, /fiducia\.cloud\/release-ledger:\s*["']fiducia-cloud\/fiducia-auth\.rs#38["']/);
  assert.match(deployment, new RegExp(`fiducia\\.cloud/release-sha: ["']${AUTH_RELEASE}["']`));
  assert.match(deployment, new RegExp(`fiducia\\.cloud/release-digest: ["']${AUTH_DIGEST}["']`));
  ...
  assert.match(deployment, /requests:[\s\S]{0,100}cpu:\s*50m[\s\S]{0,80}memory:\s*128Mi/);
  assert.match(deployment, /limits:[\s\S]{0,100}cpu:\s*["']?1["']?[\s\S]{0,80}memory:\s*512Mi/);
  ...
  assert.doesNotMatch(source, /ghcr\.io\/fiducia-cloud\/fiducia-revocation-admin:[^\s@]+/);
});
...
test('load balancer receives the reader capability only', () => {
  ...
  assert.match(loadBalancer, /dd\.dev\/fiducia-revocation-contract:\s*den-1119-reader-only-v2/);
  assert.match(
    loadBalancer,
...
test('runbook binds live evidence to the exact release and GitOps contract', async () => {

...
test('remote dev worker keeps branch-safe git setup and ssh command contracts', async () => {
  ...
  assert.match(threadTemplate, /requests:[\s\S]*memory:\s*"512Mi"/);
  assert.doesNotMatch(threadTemplate, /requests:[\s\S]*cpu:\s*"1m"/);
  ...
  assert.match(restServer, /"requests": \{ "memory": "512Mi" \}/);

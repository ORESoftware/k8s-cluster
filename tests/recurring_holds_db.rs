...
async fn on_hand(conn: &sea_orm::DatabaseConnection, pid: i64) -> i32 {
    conn.query_one(stmt("SELECT on_hand FROM inventory WHERE product_id = $1", vec![pid.into()]))
        .await.unwrap().unwrap().try_get::<i32>("", "on_hand").unwrap()
}

async fn children(conn: &sea_orm::DatabaseConnection, parent: Uuid) -> i64 {
    conn.query_one(stmt("SELECT count(*)::bigint AS n FROM orders WHERE recurs_from = $1", vec![parent.into()]))
        .await.unwrap().unwrap().try_get::<i64>("", "n").unwrap()
}

async fn due(conn: &sea_orm::DatabaseConnection, id: Uuid) -> bool {
    conn.query_one(stmt("SELECT next_run_at <= now() AS d FROM orders WHERE id = $1", vec![id.into()]))
        .await.unwrap().unwrap().try_get::<bool>("", "d").unwrap()
}
...
async fn recurring_runner_skips_stock_a_shopper_is_holding_then_fires_after_expiry() {
    ...
    let cart = db::find_or_create_cart(&conn, &CartOwner::User(shopper)).await.unwrap();
    conn.execute(stmt(
    ...
    assert_eq!(children(&conn, order).await, 0, "no child while the shopper holds stock");
    assert_eq!(on_hand(&conn, pid).await, 2, "stock untouched -- the hold was respected");
    // The cursor still advanced so the subscription isn't wedged (skip-but-advance).
    assert!(!due(&conn, order).await, "cursor advanced past now despite the skip");
    ...
    let created2 = db::run_due_recurring_orders(&conn).await.expect("runner ok");
    println!("run 2 created {created2} child(ren)");
    assert_eq!(children(&conn, order).await, 1, "fires once the hold is gone");
    ...
    conn.execute(stmt("DELETE FROM orders WHERE recurs_from = $1", vec![order.into()])).await.ok();
    conn.execute(stmt("DELETE FROM orders WHERE user_id = $1", vec![owner.into()])).await.ok();
    conn.execute(stmt("DELETE FROM carts WHERE user_id = $1", vec![shopper.into()])).await.ok();
    conn.execute(stmt("DELETE FROM stock_holds WHERE product_id = $1", vec![pid.into()])).await.ok();
    conn.execute(stmt("DELETE FROM inventory WHERE product_id = $1", vec![pid.into()])).await.ok();
    conn.execute(stmt("DELETE FROM products WHERE id = $1", vec![pid.into()])).await.ok();
}

# Agent guidelines — akrion-backend.rs

The live soccer game/learning server (vendored into k8s-cluster as the soccer-rs submodule).

Each repo is its own git repo; read its readme.md when working inside it. Work on `main` unless told otherwise.
